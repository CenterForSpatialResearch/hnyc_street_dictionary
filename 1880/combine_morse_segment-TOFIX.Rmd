...
combine_mn <- df1 %>% 
  rowwise() %>%
  #mutate(st_type = street_type_builder(Name)) %>%     # original
  
  mutate(st_type = street_type_builder(Name)) %>%
  #mutate(st_type = street_type_builder(select=c(Name))) %>%
  #mutate(st_type = street_type_builder(combine_mn[,2])) %>%
  ungroup()
